...
export class AwsIotConnector {
    ...
    public constructor(log: winston.Logger, configFile: string) {
        this.log = log;
    ...
    private subscribe(): Promise<void> {
        ...
        return this.conn.subscribe(this.config.topic, mqtt.QoS.AtLeastOnce, onPublishCallback).then((v: mqtt.MqttSubscribeRequest) => {
    ...
    private publishBridge(): Promise<void> {
        const log = this.log;
        const payload = {
            message: `Hi, am a Aloxi Bridge!`,
            timestamp: new Date().toISOString(),
            networkConnections: this.getIpAddresses()
        };
        return this.conn.publish(this.config.topic, payload, mqtt.QoS.AtLeastOnce).then((v: mqtt.MqttRequest) => {
            log.debug(`Published to topic '${this.config.topic}', packet id ${v.packet_id}`);
        });
    }
